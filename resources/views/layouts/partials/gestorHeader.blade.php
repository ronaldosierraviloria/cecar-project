    ...
    <div class="flex items-center gap-3">
        ...
        <button id="dropdownUserButton" data-dropdown-toggle="dropdownUser" data-dropdown-placement="bottom-end"
            ...
            <div class="w-9 h-9 rounded-lg flex items-center justify-center font-bold shadow-sm"
                ...
                {{ substr(Auth::user()->nombre ?? 'G', 0, 1) }}{{ substr(Auth::user()->apellido ?? '', 0, 1) }}
            </div>
            <span class="hidden sm:block text-sm font-semibold text-gray-700">{{ Auth::user()->nombre ?? 'Gestor' }}</span>
        ...
        <div id="dropdownUser" class="z-50 hidden my-2 text-base list-none bg-white divide-y divide-gray-100 rounded-2xl shadow-2xl border border-gray-100">
            <div class="px-4 py-3">
                <span class="block text-sm font-bold text-gray-800">{{ Auth::user()->nombre ?? 'Gestor' }} {{ Auth::user()->apellido ?? '' }}</span>
                <span class="block text-xs text-gray-500 truncate">{{ Auth::user()->rol ?? 'Gestor' }}</span>
            </div>
            <ul class="py-2">
                <li>
